...
/** \brief Recorre la lista y utiliza la funcion criterio recibida como parametro para devolver
 *         un valor entero.
 *         para multiples usos.
 * \param this LinkedList* Puntero a la lista
 * \param pFunc (*pFunc) Puntero a la funcion criterio
 * \return int Retorna  (-1) Error: si el puntero a la listas es NULL
                        ( 1) Si no hay elementos - OK
                        (Valor de la funcion criterio) OK
 */
int ll_mapAndReturnerInt(LinkedList* this, int (*pFunc)(void*))
{
	int returnAux = -1;
	void *pElement = NULL;
	int auxValor;
